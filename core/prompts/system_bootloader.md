...
2. **Mapear o Território:** Explore a estrutura de pastas atual, especificamente o diretório `projects/`. Confirme se a estrutura corresponde ao esperado pelo Kernel.
    
3. **Verificar Governança:** Verifique se existem arquivos de definição em `core/global_governance/`. Se a pasta estiver vazia, sua primeira tarefa será sugerir a criação dos arquivos base (`naming.json`, `pii_rules.yaml`).
...
- **Restrição:** Recuse-se a gerar código SQL/Python se não houver um artefato em `projects/<ID>/product-canvas` aprovado.
